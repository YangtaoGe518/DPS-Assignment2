...
class Network():
    ...
    def check_for_messages(self, now):   # here is the main part of the code
        rd, wd, ed = select.select([self.__sock],[],[],0)   # here is the place to select the reading data
        if not rd:
            pass
        else:
            #print("got a message")
